...
#import colorsys
#
#def whichConstructor(node):
#    for const in cic_constructors:
#        if const in node:
#            return const
#    return ''
#
#def plotGraph(graph, outfile, figsize):
#    assert(graph.order() != 0)
#    plt.figure(figsize=figsize)
#    nx.spring_layout(graph, k=1000/math.sqrt(graph.order()))
#    labels = {k:('' if '22' in k else (whichConstructor(k) if isConstructor(k) else k)) for k in g}
#    constr_colors = [(i+1)*(1.0/(len(cic_constructors)+1)) for i in range(1,len(cic_constructors)+1)]
#    nodes = list(graph.nodes())
#    colors = [colorsys.hsv_to_rgb(constr_colors[(['']+cic_constructors).index(whichConstructor(node))],.7,.7) for node in nodes]
#    nx.draw(graph, labels=labels, node_color=colors,node_size = 350)
#    plt.legend()
#    plt.savefig(outfile)
...
def exportGeoCoqDAG(max_depth, num_replicas, num_iterations, limit=None, mod_libs=False, plot=False):
    for depth in range(1, max_depth+1):
        print("depth: ", depth)
        dag = getGeoCoqDAG(depth=depth, mod_libs=mod_libs, limit=limit)
        exportOrigAndReplicas(dag, 'euclid_book', num_replicas, num_iterations, depth=depth, mod_libs=mod_libs)
