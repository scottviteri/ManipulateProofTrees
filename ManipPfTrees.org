...
#sqrt2_tree = utils.theoremNameToLists('birthday_paradox', depth=depth, mod_libs=mod_libs)
#sqrt2_dag = toplistToDAG(sqrt2_tree)
#plotGraph(nx.DiGraph(sqrt2_dag), outfile, label=True, figsize=(8,8))
outfile
#+END_SRC
...
#+BEGIN_SRC python :session :results output silent
library_name = 'Arith'
exportLibDAG(library_name, max_depth=3, num_replicas=5)
#+END_SRC
...
def plotOrigDegreeDist(theorem_name, depth, in_or_out, mod_libs=False):
    ...
    plotLineGraph(degree_dist, getDegreeOrigFileName(theorem_name, depth, in_or_out, mod_libs=mod_libs))
...
#+RESULTS:
[[file:./Images/gen_sqrt2_not_rational_d3_outdegree_dist.png]]
...
def getModularitiesFileName(theorem_name, depth):
    return './Images/'+theorem_name+'_d'+str(depth)+'_modularities.png'
...
#+RESULTS:
[[file:./Images/sqrt2_not_rational_d3_modularities.png]]
