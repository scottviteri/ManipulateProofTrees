...
gen_ev_4_dag = genComparableGraphInOut(ev_4_dag, num_iterations=100)
...
gen_dag = genComparableGraphInOut(dag, num_iterations=100) #P52 in values but not in keys
...
def exportOrigAndReplicas(theorem_dag, theorem_name, num_replicas, num_iterations, depth=2, mod_libs=False):
    ...
    gen_dags = [genComparableGraphInOut(theorem_dag, num_iterations=num_iterations) for _ in range(num_replicas)]
    #genComparableGraphs(theorem_dag, num_replicas)
...
theorem_names = [
  ...
  "CSB",
  "Alt_PI_eq",
  "equivalent_parallel_postulates_assuming_greenberg_s_postulate",
  "is_hexamy",
  "bertrand_ballot",
  "Pigeonhole",
  "four_color",
...
max_depths = [4,None,2,None,2,3,None,2,None,1,None, \
              None,1, None,None, 2,1,4,1,None,None,None, None,None]

for (theorem_name, max_depth) in zip(theorem_names, max_depths):
...
#+BEGIN_SRC python :session :results output silent
theorem_name = 'sqrt2_not_rational'
exportDAGs(theorem_name, max_depth=1, num_replicas=5, num_iterations=10000)
#+END_SRC
...
#+BEGIN_SRC python :session :results output silent
theorem_name = 'birthday_paradox'
exportDAGs(theorem_name, max_depth=4, num_replicas=5, num_iterations=10000)
#+END_SRC
...
exportDAGs(theorem_name, max_depth=2, num_replicas=5, num_iterations=10000)
#+END_SRC
...
#+BEGIN_SRC python :session :results output silent
graph = createGraph(3,.6,.33,10000)
...
#+BEGIN_SRC python :session :results file
outfile = 'Images/compare_elements_dags.png'
...
plotLineGraphsLogLog([d1,d2,d3], outfile)
outfile
#+END_SRC

#+RESULTS:
[[file:Images/compare_elements_dags.png]]


#+BEGIN_SRC python :session :results file
outfile = 'Images/compare_elements_dags_pagerank.png'

#g1 = importDAG('./ProofDAGs/euclid_book/d1.txt')
g_comp = genComparableGraphInOut(g1, num_iterations=100)
#g2 = importDAG('./ProofDAGs/gen_graph.txt')
#g3 = importDAG('./ProofDAGs/euclid_book/euclid_dependencies.txt')

#p1 = sorted(getPageRank(g1).values())[::-1]
#p_comp = sorted(getPageRank(g_comp).values())[::-1]
#p2 = sorted(getPageRank(g2).values())[::-1]
#p3 = sorted(getPageRank(g3).values())[::-1]
#plotLineGraphsLogLog([p1,p_comp,p2,p3], outfile)
outfile
#+END_SRC

#+RESULTS:
[[file:Images/compare_elements_dags_pagerank.png]]
