...
def getLabeledDescendants(dag):
    descendants = getDescendants(dag)
    return {node:{ch:descendants[ch] for ch in children} for node,children in dag.items()}

assert(getLabeledDescendants(a) ==
    {1:{2:{5,6},3:set(),4:{7}},2:{5:set(),6:set()},3:{},4:{7:set()},5:{},6:{},7:{}})

def genComparableGraph(theorem_dag, num_iterations = None):
    if not num_iterations:
        num_iterations = 10 * len(theorem_dag)
    descendants = getDescendants(theorem_dag)
    dag = nx.DiGraph(theorem_dag)
    flipped_dag = nx.DiGraph.reverse(dag)
    count = 0
    while count < num_iterations:
...
#def createGraphHighMod(m, p, q, num_iterations, modularity = 1):
#    d = nx.DiGraph({'v0':set()})
#    #memo = {'v0':set()}
...
#                        probs[connected_node] += modularity/float(distances[connected_node]+1) if connected_node != x else 1.0
#                    #print('p2',probs)
    ...
    for i in range(1,num_iterations+1):
        ...
        for j in range(m):
            if random.random() < p:
                ...
                memo[new] = memo[new].union(memo[x])
                if random.random() < q:
                    descendants = getDescendantsFromMemo(x,nx.to_dict_of_lists(d),memo)
                    #descendants = nx.algorithms.dag.descendants(d,x)
                    for descendant in descendants:
                        d.add_edge(new, descendant)
                        memo[new].add(descendant)
...
graph = createGraph(4,.56,.25,1000)
...
graph = createGraphHighMod(4,.3,.25,500)
print(getModularity(graph))
plotGraph(nx.DiGraph(graph), outfile)
outfile
#+END_SRC
...
#+BEGIN_SRC python :session :results output silent
  ...
  def getProofStatistics(theorem_name, depth):
      ...
      comparable_graphs = [genComparableGraph(theorem_dag,100) for _ in range(100)]
      shuffle_modularities = list(map(getModularity, comparable_graphs))
      nulls = [createGraph(4,.56,.25,num_nodes) for _ in range(100)]
