...
def libNameToDAG(library_name, depth=2, limit=None):
    theorem_dict = libNameToTheoremDict(library_name, depth=depth, limit=limit)
...
def exportLibDAG(library_name, max_depth, num_replicas, limit=None, plot=False):
    for depth in range(1, max_depth+1):
        dag = libNameToDAG(library_name, depth=depth, limit=limit)
        exportOrigAndReplicas(dag, library_name, num_replicas, depth=depth, plot=plot)
