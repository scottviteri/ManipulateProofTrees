...
def randomTopSort(theorem_dag):
    dag = copy.deepcopy(theorem_dag)
    L = []
    #maybe precompute indegrees, and subtract
    nodes_with_indegree = set().union(*theorem_dag.values())
    S = dag.keys() - nodes_with_indegree
    while S:
        n = S.pop()
        L.append(n)
        neighbors = dag[n].copy()
...
#+RESULTS:
[[file:<bound method Random.shuffle of <random.Random object at 0x12dd7c8>>]]
